...
class Application(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.page_lbl = ttk.Label(self.toolb_frame, textvariable=self.page_var, width=-1)
        ...
        #
        self.annot_mb.menu.add_command(label=" Image", font=("", font_size), image=self.annotf_image, compound=tk.LEFT, command=self.fadd_image)
        #
        ...
        # list of the rectanle drawn after a searching action
        self.search_id = []
        #
        # load the file
    ...
    def fadd_image(self):
        ...
        ## new binds
        # get the coords
        self.bind_id_1 = self.canvas.bind("<Button-1>", self.fadd_image1)
        # insert the image
        self.bind_id_2 = self.canvas.bind("<Button1-ButtonRelease>", self.fadd_image2)
        # RMB to invalid the choise
        self.bind_id_3 = self.canvas.bind("<Button-3>", self.fadd_image3)
        # change the cursor
        self.master.config(cursor='clock red red')
        
    # get the coords
    def fadd_image1(self, event):
        return
        cx = self.canvas.canvasx(event.x)/self.zoom
        cy = self.canvas.canvasy(event.y)/self.zoom
        # and then the rect
        rect = fitz.Rect(int(cx), int(cy), int(cx), int(cy))
        # set the var
        self.add_image_coords = rect
    
    # insert the image
    def fadd_image2(self, event):
        cx = self.canvas.canvasx(event.x)/self.zoom
        cy = self.canvas.canvasy(event.y)/self.zoom
        # and then the rect
        rect = fitz.Rect(int(cx), int(cy), int(cx)+image_size, int(cy)+image_size)
        # set the var
        self.add_image_coords = rect
        ...
        if not filename:
            self.fadd_image3(event=None)
            return
            
        try:
            rect = self.add_image_coords
            self.page.insertImage(rect, filename=filename, keep_proportion=True)
            # save the file and reload it
            self.doc.save(filename=self.filename, incremental=True)
            self.getDoc(self.filename)
            # 
            messagebox.showinfo("Info", os.path.basename(self.filename)+"\nsaved")
            # reset
            self.fadd_image3(event=None)
    ...
    # reset the binds and the pointer and the var
    def fadd_image3(self, event):
        # unbind the new
        self.canvas.unbind("<Button-1>", self.bind_id_1)
        self.canvas.unbind("<Button1-ButtonRelease>", self.bind_id_2)
        self.canvas.unbind("<Button-3>", self.bind_id_3)
    ...
    def cmousewheel(self, event):
        ...
        if self.vbar.get()[0] == 0.0:
            # load the previous page if any
            self.fminus()
        
        elif self.vbar.get()[1] == 1.0:
    ...
    def fsearch(self):
        ...
        if d.ttext == "-1" or d.ttext[0] == "":
            ...
            # 
            if d.ttext[1] == 1: 
                areas = self.page.searchFor(d.ttext[0])#, hit_max=4)
                
                for ar in areas:
                    id = self.canvas.create_rectangle(ar[0]*self.zoom, ar[1]*self.zoom, ar[2]*self.zoom, ar[3]*self.zoom)
    ...
    def fcanvas(self, rot=0):
        # the current page
        self.page_var.set("Page: {}/{}".format(self.current_page, self.page_count))
    ...
    def fplus(self):
        ...
        if self.current_page < self.page_count:
            ...
            self.canvas.yview_moveto(0.01)
            # hoping the scrolling of the page is not too fast
            bb = self.vbar_data[1] - self.vbar_data[0] - 0.01
            self.vbar.set(0.01, bb)
            # reset
            self.direction = 0
    ...
    def getText(self, fitz_rect):
        ...
        # to clipboard - se no clear aggiunge sempre al precedene
        # if 1 the use of the clipboard is disabled
        if self.annot_hl == 0:
